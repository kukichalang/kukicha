...
- **`onerr discard` on multi-return**: `w.Write(data) onerr discard` generates `_ = w.Write(data)` but `Write` returns `(int, error)`. Workaround: use `_, _ = w.Write(data)`.
- **`close` is a keyword**: Can't use `close` as a method name. Renamed to `Shutdown`.
- **`0o755` octal literals**: Not recognized by the lexer. Use `0755` instead.
...
- **Switch scope sharing**: Variables declared in different `when` branches share scope, causing redeclaration errors. Use `if/else if` instead.
- **Type inside function**: `type` declarations inside function bodies are not supported. Must be at top level.
