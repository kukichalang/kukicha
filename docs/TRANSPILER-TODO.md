...
- [x] Extract `build` into `build.go` — move flag parsing, `buildCommand()`,
  `wasmScaffold()`, `setEnvVar()`
- [x] Extract `run` into `run.go` — move flag parsing and run logic
- [x] Extract `check` into `check.go` — move flag parsing, JSON output,
  multi-target logic
- [x] Extract `version` — inlined in dispatcher (trivial one-liner)
- [x] Slim `main.go` to dispatcher: subcommand switch + `printUsage()`
- [x] Keep shared functions in `compile.go`:
  - `compile()`, `loadAndAnalyze()`, `compileResult`
  - `rewriteGoErrors()`, `rewriteVarNames()`
  - `stripFirstLine()`
- [x] Verify no test regressions — tests already use `package main` so
  they'll see all exported/unexported symbols regardless of file layout
- [x] Move `multifile_test.go` and `rewrite_errors_test.go` alongside
...
- [x] Add `brew.go` to `cmd/kukicha/` (lands cleanly after item 1)
- [x] Reuse `compile()` with `noLineDirectives: true`
- [x] Strip the `// Generated by Kukicha` header from output
- [x] Run `goimports` on the output for clean imports
- [x] Run `gofmt` on the output for canonical Go formatting
- [x] Write output to `<basename>.go` (or stdout with `--stdout`)
- [x] For directory mode: brew all `.kuki` files, optionally remove originals
  with `--remove-kuki` flag (behind confirmation prompt)
- [x] Add tests: brew example files, verify output compiles with `go build`
...
- [x] Extract lint/warning pass — `LintCandidate` collect-then-emit pattern
  in `semantic_lint.go`; all `warn()` calls replaced with `recordLint()`;
  `emitLintWarnings()` runs as final pass after type checking
- [x] Ensure existing tests pass after each extraction
