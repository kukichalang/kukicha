...
**`stdlib_registry_gen.go`** is a map of every exported stdlib function to its return-value count. The compiler's semantic analyzer uses this to correctly decompose pipe chains and `onerr` clauses. It is regenerated by `cmd/genstdlibregistry`.
