...
When `isStdlibIter` is true (or per-function for `stdlib/slice`, `stdlib/sort`, and `stdlib/concurrent`), the generator detects `any`/`any2`/`ordered`/`result` placeholders in type annotations and:
