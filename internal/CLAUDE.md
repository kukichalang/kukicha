...
`Directive` represents a `# kuki:name args...` annotation. It has `Name string`, `Args []string`, and `Token lexer.Token`. Both `FunctionDecl` and `TypeDecl` have a `Directives []Directive` field. The parser collects `TOKEN_DIRECTIVE` tokens in `skipIgnoredTokens` and attaches them to the next declaration via `drainDirectives()`.

Currently supported directives:
- `# kuki:deprecated "message"` — marks a function/type as deprecated; semantic analysis warns on calls
...
Contains two maps:
- `generatedStdlibRegistry` — function name → return count
